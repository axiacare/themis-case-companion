-- Fix team_settings RLS policies (handle existing policies properly)
...
DROP POLICY IF EXISTS "Teams can only access their own settings" ON public.team_settings;
DROP POLICY IF EXISTS "Only system admin can manage team settings" ON public.team_settings;

-- Create secure RLS policies for team_settings
